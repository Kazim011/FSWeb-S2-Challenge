...
console.log(cumleKur("Hello Wordl"));
...
function cumlelereDonustur(cumleler, ayrac=","){
	let yeniDizi= cumleler.map(deger => deger.join(ayrac));
		return yeniDizi;
}
...
for(let key in emojiler){
	...
	atılanMesaj = atılanMesaj.replaceAll(key.toUpperCase(), emojiler[key]);
	atılanMesaj = atılanMesaj.replaceAll(key , emojiler[key]);
	
}
return atılanMesaj;
}

console.log( emojileriDonustur("merhaba :p",emojiler));
